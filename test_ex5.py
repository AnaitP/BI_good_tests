...
@pytest.fixture(scope='class')
def wait_by_css(element_locator):
    wait_by_css = WebDriverWait(browser, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, element_locator)))
    return wait_by_css


class TestDirectedHierAndDD():  # Проверяем иерархию из шапки и значений таблицы и таблицы среза

    def test_hierDirected1(self, browser, wait_by_css):
        # 0+ в графе
        time.sleep(4)  # оставил специально, чтобы граф "пробесился"
        actions = webdriver.ActionChains(browser)
        wait_by_css('#slice-container-740 > svg > g:nth-child(4) > circle')
    ...
    def test_hierDirected2(self, browser, wait_by_css):
        # Красногвардейский район в графе
        time.sleep(4)  # оставил специально, чтобы граф "пробесился"
        actions = webdriver.ActionChains(browser)
        wait_by_css('#slice-container-740 > svg > g:nth-child(9) > circle')
        clickPoint = browser.find_element_by_css_selector('#slice-container-740 > svg > g:nth-child(9) > circle')
        actions.move_to_element(clickPoint).context_click().perform()
        browser.find_element_by_css_selector(
            '.d3-context-menu > ul:nth-child(1) > li:nth-child(2)').click()  # клик по меню
        time.sleep(0.5)
        wait_by_css('#slice-container-740 > svg > g:nth-child(13) > circle')
    ...
    def test_ddDirected2(self, browser, wait_by_css):
        # Красногвардейский район в графе
        time.sleep(4)
        actions = webdriver.ActionChains(browser)
        wait_by_css('#slice-container-740 > svg > g:nth-child(13) > circle')
        ...
        time.sleep(0.5)
        wait_by_css('div:nth-of-type(1) > .slice-cell .slice_container.table '
